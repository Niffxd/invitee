...
import { ButtonGroup, BUTTONS } from "./components";
import { updateInvitee, showToast } from "@/helpers";
import { FormData, FormProps } from "./types";

export const Form = ({ inviteeId, inviteeName }: FormProps) => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      await updateInvitee(
        inviteeId,
        {
          isConfirmed: data.isConfirmed as boolean,
          isDeclined: !data.isConfirmed,
            ...
            <div className="relative transform transition-all duration-300">
              ...
              >
                ...
                >
                  <Button
                    type="submit"
                    isDisabled={isSubmitting || data.isConfirmed === null}
                    className={`
                      group relative flex-1 overflow-hidden transition-all duration-500
                      active:scale-[0.98]
                      ${isSubmitting || data.isConfirmed === null ? 'opacity-50' : 'opacity-100'}
                    `}
                  >
                    {/* Button Content */}
